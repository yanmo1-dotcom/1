...
public class JackalGame extends Game {
    ...
    // ===== HUD 渲染用颜色常量/复用实例（避免每帧 new Color 产生 GC）=====
    /** 死亡横条红色（固定 alpha） */
    private static final Color DEAD_OVERLAY = new Color(0.6f, 0f, 0f, 0.6f);
    /** 任务完成横条绿色（固定 alpha） */
    private static final Color MISSION_OVERLAY = new Color(0f, 0.5f, 0f, 0.6f);
    /** GAME CLEAR 黑底（固定 alpha） */
    private static final Color CLEAR_OVERLAY = new Color(0, 0, 0, 0.8f);
    /** 过渡黑屏复用实例（alpha 动态变化，每帧 set 后复用） */
    ...
    private void renderHud() {
        ...
        if (lm.getFadeAlpha() > 0f) {
            ...
            shapes.updateMatrices();
            shapes.begin(ShapeRenderer.ShapeType.Filled);
            fadeColor.a = lm.getFadeAlpha();
            shapes.setColor(fadeColor);
