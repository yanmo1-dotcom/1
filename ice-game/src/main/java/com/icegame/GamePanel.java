...
final class GamePanel extends JPanel implements ActionListener {
    ...
    GamePanel(GameFrame frame, Mode mode, String host) {
        ...
        setFocusable(true);
        setBackground(new Color(218, 244, 255));
        bindKeys();
    ...
    private void readState(String line) {
        ...
        try {
            synchronized (state) {
                ...
                if (mode != Mode.CLIENT) {
                    state.rightX = Double.parseDouble(p[7]);
                    state.rightY = Double.parseDouble(p[8]);
                }
    ...
    @Override protected void paintComponent(Graphics raw) {
        ...
        paintRink(g);
        synchronized (state) {
            paddle(g, state.leftX, state.leftY, new Color(20, 112, 190));
            paddle(g, state.rightX, state.rightY, new Color(224, 55, 62));
            g.setColor(new Color(22, 30, 39));
